...
class NVCFSingularityProvider(Provider):
    ...
    def start_emulator(self, path_to_vm: str, headless: bool, os_type: str = "Ubuntu"):
        ...
        try:
            ...
            runtime_lib_dir = runtime_env.get("OSWORLD_RUNTIME_LIB_DIR")
            if runtime_lib_dir and os.path.isdir(runtime_lib_dir):
                runtime_env["LD_LIBRARY_PATH"] = (
                    f"{runtime_lib_dir}:{runtime_env.get('LD_LIBRARY_PATH', '')}"
                )

            path_prefixes = []
            for env_name in ("OSWORLD_RUNTIME_BIN_DIR", "OSWORLD_APPTAINER_LIBEXEC_DIR"):
                path = runtime_env.get(env_name)
                if path and os.path.isdir(path):
                    path_prefixes.append(path)
            if path_prefixes:
                runtime_env["PATH"] = ":".join(path_prefixes + [runtime_env.get("PATH", "")])
