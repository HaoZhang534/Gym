...
class PythonController:
    ...
    def execute_python_command(self, command: str) -> None:
        ...
        command_list = ["python", "-c", self.pkgs_prefix.format(command=command)]
        action_timeout_sec = float(os.environ.get("OSWORLD_ACTION_TIMEOUT_SEC", "0") or 0)
        if action_timeout_sec > 0:
            # GNU timeout creates a separate process group and terminates the
            # whole generated action tree. This matters when model code starts
            # grandchildren (for example subprocess.run(find ...)); killing
            # only the outer python process can leave the child holding stdout
            # open and stall the desktop server indefinitely.
            command_list = [
                "timeout",
                "--kill-after=5s",
                f"{action_timeout_sec:g}s",
                *command_list,
            ]
        payload = json.dumps({"command": command_list, "shell": False})
        request_timeout = min(120.0, action_timeout_sec + 15.0) if action_timeout_sec > 0 else 120.0
    ...
    def start_recording(self):
        ...
        """
        logger.info("Skipping recording start.")
        return
        for _ in range(self.retry_times):
            try:
    ...
    def end_recording(self, dest: str):
        ...
        """
        logger.info("Skipping recording end.")
        return
        for _ in range(self.retry_times):
            try:
